...
public class BankAccountDAO {
    ...
    final String SELECT_USER_BY_NAME = "SELECT account_number, balance, email, phone_number FROM bankDB WHERE customer_name = ?";
    final String SELECT_USER_BY_EMAIL = "SELECT account_number, customer_name, balance, phone_number FROM bankDB WHERE email = ?";
    final String SELECT_USER_BY_PHONE = "SELECT account_number, customer_name, balance, email FROM bankDB WHERE phone_number = ?";
    final String SELECT_BALANCE_BY_ACCT = "SELECT balance FROM bankDB WHERE account_number = ?";
    final String SELECT_BALANCE_BY_NAME = "SELECT balance FROM bankDB WHERE customer_name = ?";
    ...
    final String UPDATE_BALANCE_CN = "UPDATE bankDB SET balance = ? where customer_name = ?";

    final String DELETE_BY_ACCT = "DELETE FROM bankDB WHERE account_number = ?";
    final String DELETE_BY_NAME = "DELETE FROM bankDB WHERE customer_name = ?";
    ...
    public void readAccountByCustomerName(String customer_name) {
        try {
            Connection conn = getConnection();
            PreparedStatement prepStmt = conn.prepareStatement(SELECT_USER_BY_NAME);
    ...
    public void readAccountByEmail(String email) {
        try {
            Connection conn = getConnection();
            PreparedStatement prepStmt = conn.prepareStatement(SELECT_USER_BY_EMAIL);
    ...
    public void readAccountByPhoneNumber(String phone_number) {
        try {
            Connection conn = getConnection();
            PreparedStatement prepStmt = conn.prepareStatement(SELECT_USER_BY_PHONE);
    ...
    public void depositByCustomerName(String customer_name, double deposit) {
        try {
            ...
            PreparedStatement balanceStmt = conn.prepareStatement(SELECT_USER_BY_NAME);
            balanceStmt.setString(1, customer_name);
            ResultSet rs = balanceStmt.executeQuery();
            rs.next();
            double balance = rs.getDouble("balance");

            PreparedStatement prepStmt = conn.prepareStatement(UPDATE_BALANCE_CN);
            prepStmt.setDouble(1, balance + deposit);
            prepStmt.setString(2, customer_name);
            if (prepStmt.executeUpdate() == 1) {
                System.out.println("Successfully deposited!");
            }
        } catch (SQLException e) {
    ...
    public void withdrawByCustomerName(String customer_name, double withdraw) {
        try {
            ...
            PreparedStatement balanceStmt = conn.prepareStatement(SELECT_BALANCE_BY_NAME);
            balanceStmt.setString(1, customer_name);
            ResultSet rs = balanceStmt.executeQuery();
            rs.next();
            double balance = rs.getDouble("balance");
    ...
    public void deleteAccountByCustomerName(String customer_name) {
        try {
            Connection conn = getConnection();
            PreparedStatement prepStmt = conn.prepareStatement(DELETE_BY_NAME);
            prepStmt.setString(1, customer_name);
            if (prepStmt.executeUpdate() == 1) {
                System.out.println("Successfully deleted!");
            }
